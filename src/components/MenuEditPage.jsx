...
const MenuEditPage = (props) => {
  ...
  const [type, setType] = useState(() => {
    if (props.data.category?.id == "1") return "Đồ ăn";
    else return "Thức uống";
  });
  const [numOfI, setNumOfI] = useState(props.data.menuDetails?.length);
  const [idType, setIdType] = useState(props.data?.category.id);
  const [ingredients, setIngredients] = useState(props.listIngredient);
  // list ingredients in DB
  const [listIngredients, setListIngredients] = useState(
    props.data?.menuDetails
  );
  ...
  const [data, setData] = useState(() => {
    return props.data.menuDetails?.map((ingre) => {
      return {
        id: ingre.product.id,
  ...
  const contentPut = (a) => {
    if (a.length) {
      ...
      } else toast.error("Vui lòng nhập số lượng của nguyên liệu phù hợp");
    } else toast.error("Số lượng nguyên liệu không hợp lệ");
  };

  const handleUpdate = (e) => {
    axios
      .put(`menu/${props.data.id}`, e, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })
